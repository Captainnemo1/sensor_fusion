...
class LidarGUI:
    def __init__(self, master):
        ...
        self.start_button = tk.Button(self.master,  bg="black", fg="black", font=("Arial", 24))
        self.start_button.pack()

        self.pause_button = tk.Button(self.master, text="Pause", bg="red", fg="white", font=("Arial", 24), command=self.pause)
        ...
        self.lidar = None
        self.channel = None
        self.lidar_thread = None  # Variable to hold the lidar thread
        self.paused = False
    ...
    def start(self):
        if not self.lidar_thread or not self.lidar_thread.is_alive():
            self.lidar_thread = threading.Thread(target=self.run)
    ...
    def pause(self):
        ...
        if self.paused:
            ...
            self.lidar_status_label.config(text="Lidar Status: Running")
            self.ultrasonic_status_label.config(text="Ultrasonic Status: Running")
            # Resume the lidar iteration if it was paused
            self.resume_lidar_iteration()

    def resume_lidar_iteration(self):
        if self.lidar and self.paused:
            self.paused = False
            try:
                for val in self.lidar.iter_measures():
                    if self.paused:  # If paused during iteration, break the loop
                        break
                    # ... (remaining lidar iteration code)
            except KeyboardInterrupt:
                pass
            finally:
