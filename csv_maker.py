...
class LidarGUI:
    ...
    def save_to_csv(self, lidar_distance, ultrasonic_distance):
        csv_file = 'dist.csv'
        max_entries = 5
        ...
        # Remove old entries if the number exceeds max_entries
        if len(existing_entries) > max_entries:
            existing_entries = existing_entries[-(max_entries - 1):]
            
        
        # Add the new data
        new_data = [lidar_distance, ultrasonic_distance]
        existing_entries.append(new_data)

    # Write the updated entries to the CSV file
        with open(csv_file, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['Lidar Distance', 'Ultrasonic Distance'])
                ...
                for val in self.lidar.iter_measures():
                    if val[3] != 0:
                        if self.find_zero_front(val[2], val[3]):
                            ...
                            # Save Lidar and Ultrasonic distances to CSV
                            self.save_to_csv(lidar_distance, ultrasonic_distance)
